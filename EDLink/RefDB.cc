...
string* RefDB::globalReference() {
    string *globalRef = new string;

    for (int i = 0; i < refPos.size(); ++i) {
        loadChromo(i);
        boost::to_upper(chromosome);

        for (unsigned long long j = 0; j < chromosome.size(); ++j) {
            if (chromosome[j] != 'A' && chromosome[j] != 'C' && chromosome[j] != 'G' && chromosome[j] != 'T')
                chromosome[j] = 'N';
        }

        globalRef->append(chromosome);
        globalRef->append("|");
    }
